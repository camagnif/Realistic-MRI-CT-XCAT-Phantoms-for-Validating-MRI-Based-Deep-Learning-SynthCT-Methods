...
if desired=='train':
    id_ph = [76, 77, 80, 89, 92, 96, 98, 106, 108, 117, 118, 128, 139, 140, 141, 142, 144, 145, 146, 147, 148, 149, 150, 151, 152, 153, 154, 155, 157, 159, 162, 163, 164, 166, 167, 168, 169, 170, 171, 173, 175, 176, 178, 180, 182, 184, 196, 200, 201]
elif desired=='test':
    id_ph = [50, 51, 71, 86, 99, 143]
elif desired=='all':
    id_ph = [50, 51, 71, 76, 77, 80, 86, 89, 92, 96, 98, 99, 106, 108, 117, 118, 128, 139, 140, 141, 142, 143, 144, 145, 146, 147, 148, 149, 150, 151, 152, 153, 154, 155, 157, 159, 162, 163, 164, 166, 167, 168, 169, 170, 171, 173, 175, 176, 178, 180, 182, 184, 196, 200, 201]

folder_COMBAT = '//NAS-CARTCAS/camagnif/tesi/Data/mri/results_revisori/6th/test/inference/all_volumes_COMBAT/'
folder_MRI_corrected = '//NAS-CARTCAS/camagnif/tesi/Data/mri/results_revisori/old/'
folder_Mask = '//NAS-CARTCAS/camagnif/tesi/Data/mri/results_revisori/6th/test/inference/all_Masks/'
folder_XCAT = '//NAS-CARTCAS/camagnif/tesi/FINAL_RESULTS/CT/train/from348to320/XCAT/'

#%%

# UNPAIRED
folder_unpaired = '//NAS-CARTCAS/camagnif/tesi/Data/mri/results_revisori/3rd/test/inference/test_volumes/unpaired/'

#_____________________________________HISTOGRAM________________________________

folder_all_fake = '//NAS-CARTCAS/camagnif/tesi/Data/ct/results_revisori/3rd/test/inference/test_volumes/unpaired/MRI_fake_all/'
folder_all_pz = '//NAS-CARTCAS/camagnif/tesi/Data/ct/results_revisori/3rd/test/inference/test_volumes/unpaired/MRI_real_all/'
folder_phantom = '//NAS-CARTCAS/camagnif/tesi/Data/ct/results_revisori/3rd/test/inference/test_volumes/unpaired/Phantoms_recon_all/'
folder = '//NAS-CARTCAS/camagnif/tesi/Data/ct/results_revisori/8th/inference/'
...
def save_xlsx_paired(path, id_phantoms, MAE_list, MAE_air_list, MAE_bone_list, MAE_soft_list, SSIM_list, FSIM_list, EPR_list, EGR_list, LPIPS_list):
    ...
    for j in range(1, max_column + 1):
        ...
        if HEADER.value == "MAE_air":
            for i in range(dim):
                cell = sheet.cell(max_row + 1 + i, j)
                cell.value = MAE_air_list[i]
            mean_cell = sheet.cell(max_row + 1 + dim, j)
            mean_cell.value = np.mean(MAE_air_list)
            std_cell = sheet.cell(max_row + 2 + dim, j)
            std_cell.value = np.std(MAE_air_list)
            print("MAE AIR Done!")
            
        if HEADER.value == "MAE_bone":
            for i in range(dim):
                cell = sheet.cell(max_row + 1 + i, j)
                cell.value = MAE_bone_list[i]
            mean_cell = sheet.cell(max_row + 1 + dim, j)
            mean_cell.value = np.mean(MAE_bone_list)
            std_cell = sheet.cell(max_row + 2 + dim, j)
            std_cell.value = np.std(MAE_bone_list)
            print("MAE BONE Done!")
        
        if HEADER.value == "MAE_soft":
            for i in range(dim):
                cell = sheet.cell(max_row + 1 + i, j)
                cell.value = MAE_soft_list[i]
            mean_cell = sheet.cell(max_row + 1 + dim, j)
            mean_cell.value = np.mean(MAE_soft_list)
            std_cell = sheet.cell(max_row + 2 + dim, j)
            std_cell.value = np.std(MAE_soft_list)
            print("MAE SOFT Done!")
        ...
        if HEADER.value == "LPIPS":
            for i in range(dim):
                cell = sheet.cell(max_row + 1 + i, j)
                cell.value = LPIPS_list[i]
            mean_cell = sheet.cell(max_row + 1 + dim, j)
            mean_cell.value = np.mean(LPIPS_list)
            std_cell = sheet.cell(max_row + 2 + dim, j)
            std_cell.value = np.std(LPIPS_list)
            print("LPIPS Done!")
...
def airBoneSoft(inputCT):
    THR_AIR = -700
    THR_BONE = 750
    
    inpArray = inputCT.numpy()
    
    maskBody = inpArray > -1000  # 1 inside the body, 0 outside
    maskAir = (inpArray < THR_AIR) & maskBody
    maskBone = inpArray > THR_BONE
    maskSoft = (~maskAir) & (~maskBone) & maskBody  # inside body, not air, not bone
    
    maskAir = tf.convert_to_tensor(maskAir.astype(int), dtype=tf.float32)
    maskBone = tf.convert_to_tensor(maskBone.astype(int), dtype=tf.float32)
    maskSoft = tf.convert_to_tensor(maskSoft.astype(int), dtype=tf.float32)

    return maskAir, maskBone, maskSoft
...
for index in id_ph:
    ...
    dir_fakeMRI = folder_MRI_corrected + str(index) +  '_fakeMRI.mha'#'_fakeMRI_118_train_corr_prova_320x260_new.mha'
    fakeMRI = load(dir_fakeMRI)
    dir_mask = folder_Mask + str(index) + '_mask.mha'
    vol_mask = load(dir_mask)
    dir_XCAT = folder_XCAT + str(index) + '_HU_cut.mha'
    XCAT = load(dir_XCAT)
    maskA, maskB, maskS = airBoneSoft(XCAT)
    ...
    #for slice in range(0, 2):
        ...
        slice_maskA = extract_slice(maskA, slice)
        slice_maskB = extract_slice(maskB, slice)
        slice_maskS = extract_slice(maskS, slice)
    #for slice in range(0, fakeMRI.shape[2]):
        #print(extract_slice(COMBAT, slice))
        slice_combat_ssim = pad_to_shape(extract_slice(COMBAT, slice))
        slice_fakeMRI_ssim = pad_to_shape(extract_slice(fakeMRI, slice))
        slice_mask_ssim = pad_to_shape(extract_slice(vol_mask, slice))
        slice_maskA_ssim = pad_to_shape(extract_slice(maskA, slice))
        slice_maskB_ssim = pad_to_shape(extract_slice(maskB, slice))
        slice_maskS_ssim = pad_to_shape(extract_slice(maskS, slice))
        
        #assert slice_combat.shape == (348, 348, 1)
        #assert slice_fakeMRI.shape == (348, 348, 1)
        #assert slice_mask.shape == (348, 348, 1)
        #assert slice_maskA.shape == (348, 348, 1)
        #assert slice_maskB.shape == (348, 348, 1)
        #assert slice_maskS.shape == (348, 348, 1)

        mae = mean_absolute_error(slice_fakeMRI, slice_combat, slice_mask)
        #print('MAE: ', mae)
        MAE.append(mae)
        mae_air = mean_absolute_error(slice_fakeMRI, slice_combat, slice_maskA)
        #print('MAE air: ', mae_air)
        if not np.isnan(mae_air):        
            MAE_air.append(mae_air)
        mae_bone = mean_absolute_error(slice_fakeMRI, slice_combat, slice_maskB)
        #print('MAE bone: ', mae_bone)
        if not np.isnan(mae_bone):
            MAE_bone.append(mae_bone)
        mae_soft = mean_absolute_error(slice_fakeMRI, slice_combat, slice_maskS)
        #print('MAE soft: ', mae_soft)
        MAE_soft.append(mae_soft)
        
        slice_combat = normalize(slice_combat)
        slice_fakeMRI = normalize(slice_fakeMRI)
        
        # SSIM and FSIM
        ssim_value = ssim(slice_fakeMRI_ssim.numpy(), slice_combat_ssim.numpy(), 2)
        ...
        sobel_filter = tf.constant([[1, 2, 1], [0, 0, 0], [-1, -2, -1]], dtype=tf.float32)
        img1_edges = tf.nn.conv2d(np.expand_dims(img1, 0), tf.expand_dims(tf.expand_dims(sobel_filter, -1), -1), strides=[1, 1, 1, 1], padding='SAME')
        # plt.imshow(tf.squeeze(img1_edges))
        # plt.show()
        img2_edges = tf.nn.conv2d(np.expand_dims(img2, 0), tf.expand_dims(tf.expand_dims(sobel_filter, -1), -1), strides=[1, 1, 1, 1], padding='SAME')
        # plt.imshow(tf.squeeze(img2_edges))
        # plt.show()
        edge_gt = np.abs(tf.squeeze(img1_edges))
        # plt.imshow(edge_gt)
        # plt.show()
        ...
        threshold2 = 0.9
        _, binary_img_gt = cv2.threshold(edge_gt, threshold, 1, cv2.THRESH_BINARY)
        # plt.imshow(binary_img_gt)
        # plt.show()
        _, binary_img_pred = cv2.threshold(edge_pred, threshold2, 1, cv2.THRESH_BINARY)
        # plt.imshow(binary_img_pred)
        # plt.show()
        inter = cv2.bitwise_and(binary_img_gt, binary_img_pred) # edge map in common between edges1 and edges2
        inter = np.abs(inter)
        _, binary_img_inter = cv2.threshold(inter, threshold * 0.625, 1, cv2.THRESH_BINARY)  # 0.625 è un valore approssimato per mantenere l'intersezione valida
        # plt.imshow(binary_img_inter)
        # plt.show()
        tot_gt = np.sum(binary_img_gt)
        tot_pred = np.sum(binary_img_pred)
        tot_inter = np.sum(binary_img_inter)
        ...
        # --- LPIPS Computation ---
        # Extract centered patch: from 320x260, extract the patch 256x256 in the center, corresponding patch between slice_COMBAT (gt) and slice_fakeMRI (pred)
        slice_combat = tf.squeeze(slice_combat)
        slice_fakeMRI = tf.squeeze(slice_fakeMRI)
        H, W = slice_combat.shape
        start_y = (H - 256) // 2
        start_x = (W - 256) // 2
        patch_combat = slice_combat[start_y:start_y + 256, start_x:start_x + 256]
        #plt.imshow(patch_combat)
        #plt.show()
        patch_fake = slice_fakeMRI[start_y:start_y + 256, start_x:start_x + 256]
        #plt.imshow(patch_fake)
        #plt.show()
        
        # Convert to torch tensor and normalize
        patch_combat = torch.tensor(patch_combat.numpy(), dtype=torch.float32)
        patch_fake = torch.tensor(patch_fake.numpy(), dtype=torch.float32)
        patch_combat = (patch_combat - patch_combat.min()) / (patch_combat.max() - patch_combat.min() + 1e-8)
        patch_fake = (patch_fake - patch_fake.min()) / (patch_fake.max() - patch_fake.min() + 1e-8)
        patch_combat = patch_combat * 2 - 1
        patch_fake = patch_fake * 2 - 1
        patch_combat = patch_combat.unsqueeze(0).repeat(3, 1, 1).unsqueeze(0)
        patch_fake = patch_fake.unsqueeze(0).repeat(3, 1, 1).unsqueeze(0)
        
        # Compute LPIPS
        lpips_value = loss_fn(patch_fake, patch_combat)
    ...
    print('mean MAE for phantom ', index, ' is: ', np.mean(MAE))
    MAE_list.append(np.mean(MAE))
    print('mean MAE air for phantom ', index, ' is: ', np.mean(MAE_air))
    MAE_air_list.append(np.mean(MAE_air))
    print('mean MAE bone for phantom ', index, ' is: ', np.mean(MAE_bone))
    MAE_bone_list.append(np.mean(MAE_bone))
    print('mean MAE soft for phantom ', index, ' is: ', np.mean(MAE_soft))
    MAE_soft_list.append(np.mean(MAE_soft))
    print('mean SSIM for phantom ', index, ' is: ', np.mean(SSIM))
    SSIM_list.append(np.mean(SSIM))
    print('mean FSIM for phantom ', index, ' is: ', np.mean(FSIM))
    FSIM_list.append(np.mean(FSIM))
    print('mean EPR for phantom ', index, ' is: ', np.mean(EPR))
    EPR_list.append(np.mean(EPR))
    print('mean EGR for phantom ', index, ' is: ', np.mean(EGR))
    EGR_list.append(np.mean(EGR))
    print('mean LPIPS for phantom ', index, ' is: ', np.mean(LPIPS))
    LPIPS_list.append(np.mean(LPIPS))
...
path = '//NAS-CARTCAS/camagnif/tesi/Data/mri/results_revisori/old/fakeMRI_paired_old_metrics_320x260_test.xlsx'
save_xlsx_paired(path, id_ph, MAE_list, MAE_air_list, MAE_bone_list, MAE_soft_list, SSIM_list, FSIM_list, EPR_list, EGR_list, LPIPS_list)
...
print('MAE AIR medio vale ', np.mean(MAE_air_list), ' con std ', np.std(MAE_air_list))
print('MAE BONE medio vale ', np.mean(MAE_bone_list), ' con std ', np.std(MAE_bone_list))
print('MAE SOFT medio vale ', np.mean(MAE_soft_list), ' con std ', np.std(MAE_soft_list))
print('LPIPS medio vale ', np.mean(LPIPS_list), ' con std ', np.std(LPIPS_list))
...
for num, id in enumerate(id_ph):
    ...
    for folder in list_folders:
        if folder.__contains__(str(id)):
            ...
            ph = folder_data + str(id) + '_fakeMRI.mha'
...
path = '//NAS-CARTCAS/camagnif/tesi/Data/mri/results_revisori/3rd/test/inference/test_volumes/fakeMRI_unpaired_3rd_test_metrics.xlsx'
...
CT_fake = glob.glob(folder_all_fake + '*.mha')
CT_real = glob.glob(folder_all_pz + '*.mha')
ph = glob.glob(folder_phantom + '*.mha')
assert len(CT_fake) == len(CT_real) == len(ph), 'No matching dimensions'

bins = 70
hist_sum_fake = np.zeros(bins)
hist_sum_real = np.zeros(bins)
#hist_sum = np.zeros(bins)
hist_sum = np.zeros(1000)

# FAKE
for i in range(len(CT_fake)): 
    CT = load(CT_fake[i])
    #MRI = MRI.flatten()
    hist, bin_edges_f = np.histogram(CT, bins=bins, range=(-1000, 1000))
    hist_normalized = hist / np.sum(hist)
    hist_sum_fake += hist_normalized
# Mean histogram
mean_hist_fake = hist_sum_fake / len(CT_fake)

# REAL PZ
for i in range(len(CT_real)): 
    CT = load(CT_real[i])
    #MRI = MRI.flatten()
    hist, bin_edges_r = np.histogram(CT, bins=bins, range=(-1000, 1000))
    hist_normalized = hist / np.sum(hist)
    hist_sum_real += hist_normalized
# Mean histogram
mean_hist_real = hist_sum_real / len(CT_real)

# PHANTOMS
for i in range(len(ph)): 
    phantom = load(ph[i])
    #phantom = phantom.flatten              # 1000
    hist, bin_edges_p = np.histogram(phantom, bins=1000, range=(-1000, 1000))
    hist_normalized = hist / np.sum(hist)
    hist_sum += hist_normalized
# Mean histogram
mean_hist = hist_sum / len(ph)
...
plt.figure(figsize=(15, 10))
plt.rcParams['font.family'] = 'serif'
plt.plot(bin_edges_f[:-1], mean_hist_fake, color='coral', linestyle='-', linewidth=2, label='Realistic CT')
plt.fill_between(bin_edges_f[:-1], mean_hist_fake, color='coral', alpha=0.3, edgecolor='coral')
plt.plot(bin_edges_r[:-1], mean_hist_real, color='darkviolet', linestyle='-', linewidth=2, label='Real CT')
plt.fill_between(bin_edges_r[:-1], mean_hist_real, color='darkviolet', alpha=0.3, edgecolor='darkviolet')
plt.plot(bin_edges_p[:-1], mean_hist, color='mediumseagreen', linestyle='-', linewidth=2, label='XCAT')
#plt.fill_between(bin_edges_p[:-1], mean_hist, color='mediumseagreen', alpha=0.3, edgecolor='mediumseagreen')
plt.ylabel('Relative frequency', fontsize=20)
plt.xlabel('Voxel Intensity [a.u.]', fontsize=20)
plt.xticks(fontsize=20)
plt.yticks(fontsize=20)
plt.legend(fontsize=20)
plt.title('Realistic CT vs Real CT vs XCAT', fontsize=24)
plt.tight_layout()
plt.savefig(folder + 'fakeCT_vs_realCT_vs_XCAT_histogram_comparison.png')
